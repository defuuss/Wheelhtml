(() => {
  ...
  function collectUpstream(id, byId, found = new Set()) {
    depState(id).requires.forEach(ref => {
      ...
      found.add(ref);
      collectUpstream(ref, byId, found);
    });
    return found;
  }
  ...
  function collectDownstream(id, reverse, found = new Set()) {
    (reverse.get(id) || []).forEach(ref => {
      ...
      found.add(ref);
      collectDownstream(ref, reverse, found);
    });
    return found;
  }
  ...
  function updateStatusRow(card, byId, reverse) {
    ...
    if (state.requires.length) {
      ...
      row.appendChild(makeChip(
        state.mode === 'any' ? `🔗 After ANY: ${shortNames}` : `🔗 After: ${shortNames}`,
        'requires',
        `${state.mode === 'any' ? 'Available after ANY of' : 'Available after ALL of'}: ${reqNames.join(separator)}`
      ));
    } else {
      row.appendChild(makeChip('● With group', 'ready', 'Available whenever this group is active'));
    }
  ...
  function enhanceCard(card) {
    if (card.dataset.visualPathBound !== '1') {
      ...
      card.addEventListener('mouseleave', event => {
        ...
        if (activeCard === card) clearPathHighlight();
      });
  ...
  function refresh() {
    ...
    byId.forEach(card => {
      enhanceCard(card);
      updateStatusRow(card, byId, reverse);
    });
