(() => {
  ...
  function injectEditorUi() {
    ...
    motion.innerHTML = `<div class="section-kicker">MOTION</div><h3>Spin-up & spin-down</h3><p class="muted">Each spin randomly chooses an acceleration and slowdown time inside these ranges. The remaining part of the configured total spin time is the full-speed cruise.</p><div class="spin-phase-track"><b>START</b><span class="arrow"></span><b>↗ FULL SPEED</b><span class="arrow"></span><b>↘ STOP</b></div><strong class="mini-heading">Spin-up / acceleration</strong><div class="spin-mini-grid"><label class="field">Minimum spin-up seconds<input id="spinUpMinSeconds" type="number" min="0.1" max="10" step="0.1"></label><label class="field">Maximum spin-up seconds<input id="spinUpMaxSeconds" type="number" min="0.1" max="12" step="0.1"></label></div><strong class="mini-heading" style="margin-top:10px">Spin-down / slowdown</strong><div class="spin-mini-grid"><label class="field">Minimum spin-down seconds<input id="spinDownMinSeconds" type="number" min="0.3" max="20" step="0.1"></label><label class="field">Maximum spin-down seconds<input id="spinDownMaxSeconds" type="number" min="0.3" max="25" step="0.1"></label></div><div id="spinMotionRange" class="spin-live-range"></div><div class="spin-range-note">The wheel accelerates smoothly from rest, holds a constant maximum speed, then decelerates smoothly to zero. No speed jump between phases.</div>`;
    ...
    drama.innerHTML = `<div class="section-kicker">DRAMA</div><h3>Random spin & suspense</h3><p class="muted spin-settings-note">Drama mode never re-spins the wheel. After the normal spin-down it can fake a stop and creep a little farther before the real result.</p><label class="field">Maximum full rotations<input id="maxTurns" type="number" min="3" max="24" step="1"></label><label class="drama-master"><span><strong>Drama mode</strong><small>Randomly fake an almost-stop, then continue slowly for a short distance.</small></span><input id="dramaEnabled" class="drama-toggle" type="checkbox"><span id="dramaStatus" class="drama-status">ON</span></label><div id="dramaControls" class="drama-controls"><label class="field">Drama chance (%)<input id="dramaChance" type="number" min="0" max="100" step="1"></label><div class="spin-mini-grid"><label class="field">Continue from (degrees)<input id="dramaCreepMinDegrees" type="number" min="5" max="160" step="5"></label><label class="field">Continue to (degrees)<input id="dramaCreepMaxDegrees" type="number" min="5" max="180" step="5"></label></div><div class="spin-range-note">Example: 30–80° means only part of one turn after the fake stop.</div></div><label class="check-line"><input id="showSlowIcon" type="checkbox"> <span>Show the current pictogram above the wheel while it is slowing</span></label><label class="field">Start live pictograms after (%)<input id="iconPreviewStartPercent" type="number" min="10" max="80" step="5"></label><div class="spin-range-note">Lower values make the large current pictogram start earlier.</div>`;
    grid.appendChild(drama);

    const ids = ['maxTurns','spinUpMinSeconds','spinUpMaxSeconds','spinDownMinSeconds','spinDownMaxSeconds','dramaEnabled','dramaChance','dramaCreepMinDegrees','dramaCreepMaxDegrees','showSlowIcon','iconPreviewStartPercent'];
    ...
    const updateDramaUi = () => {
      ...
      ['dramaChance','dramaCreepMinDegrees','dramaCreepMaxDegrees'].forEach(id => { const el = document.getElementById(id); if (el) el.disabled = !enabled; });
    };
    ...
    const sync = () => {
      const s = load();
      ids.forEach(id => { const el = document.getElementById(id); if (!el) return; if (el.type === 'checkbox') el.checked = !!s[id]; else el.value = s[id]; });
      const minTurns = Number(document.getElementById('minTurns')?.value || 6);
      if (Number(document.getElementById('maxTurns')?.value || 0) < minTurns) document.getElementById('maxTurns').value = Math.min(24, minTurns + 2);
      updateDramaUi();
      updateRangeNote();
    };
    ...
    const persist = () => {
      ...
      const s = save({maxTurns:Math.round(clamp(document.getElementById('maxTurns')?.value,minTurns,24,Math.max(9,minTurns))),spinUpMinSeconds:upMin,spinUpMaxSeconds:upMax,spinDownMinSeconds:downMin,spinDownMaxSeconds:downMax,dramaEnabled:!!document.getElementById('dramaEnabled')?.checked,dramaChance:document.getElementById('dramaChance')?.value,dramaCreepMinDegrees:creepMin,dramaCreepMaxDegrees:creepMax,showSlowIcon:!!document.getElementById('showSlowIcon')?.checked,iconPreviewStartPercent:document.getElementById('iconPreviewStartPercent')?.value});
      ids.forEach(id => { const el = document.getElementById(id); if (!el) return; if (el.type === 'checkbox') el.checked = !!s[id]; else el.value = s[id]; });
      updateDramaUi(); updateRangeNote();
    };

    ids.forEach(id => { const el = document.getElementById(id); el?.addEventListener(el.type === 'checkbox' ? 'change' : 'input', persist); });
    ['minSpin','maxSpin','minTurns'].forEach(id => document.getElementById(id)?.addEventListener('input', () => { if (id === 'minTurns') persist(); else updateRangeNote(); }));
