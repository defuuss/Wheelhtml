...
(async()=>{
 for(const eventType of ['cardPick','randomize','spinAgain']){
  ...
  try{
   ...
   if(eventType==='cardPick'){assert.equal(M.loadSession(M.loadConfig()).history.length,0);w.document.getElementById('resultCloseBtn').click();assert.equal(w.document.getElementById('fateDeckOverlayV4').hidden,false);assert.match(w.document.getElementById('fateDeckCount').textContent,/20/);}
