...
(async()=>{
 ...
 assert.equal(w.document.getElementById('spinMode'),null);assert.equal(roundtrip.settings.spinMode,'manual');
 const button=w.document.getElementById('spinBtn');button.click();await pause(1300);
 assert.equal(button.querySelector('.spin-label').textContent,'STOP');assert.equal(button.disabled,false);
 ...
 assert.equal(w.FortuneModel.loadSession(cfg).completedLevels.start,undefined);w.FortunePlay.acceptPending();
 ...
 assert.match(w.document.getElementById('resultTimerPanel').textContent,/02:00|2:00/);assert.deepEqual(errors,[]);
