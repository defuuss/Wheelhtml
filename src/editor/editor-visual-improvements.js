(() => {
  ...
  if (window.__fortuneEditorVisualImprovementsV3) return;
  window.__fortuneEditorVisualImprovementsV3 = true;
  ...
  function cardMap() {
    return new Map(cards().map(card => [card.dataset.id, card]));
  }
  ...
  function depState(id) {
    try {
      ...
      return {
        mode: raw.requiresMode === 'any' ? 'any' : 'all',
        requires: [...new Set((raw.requiresForfeitIds || []).filter(Boolean))]
      };
    } catch (_) {
      return { mode: 'all', requires: [] };
    }
  }
  ...
  function reverseMap(byId) {
    ...
    byId.forEach((_, id) => {
      depState(id).requires.forEach(ref => {
        if (reverse.has(ref)) reverse.get(ref).push(id);
      });
    });
    return reverse;
  }
  ...
  function eventLabel(value) {
    return ({
      ...
      unlock: '🔓 Unlock event',
      doubleSpin: '×2 Double spin',
      immunity: '🛡 Immunity',
      randomize: '🎲 Randomize',
      goodCard: '🟢 Fate draw',
      badCard: '🔴 Fate draw',
      doubleOrNothing: '🟡 Fate draw'
    })[value] || '';
  }
  ...
  function buildChips(data) {
    ...
    if (data.reqNames.length) {
      const separator = data.mode === 'any' ? ' / ' : ' + ';
      const shortNames = data.reqNames.slice(0, 2).join(separator) + (data.reqNames.length > 2 ? ` +${data.reqNames.length - 2}` : '');
      chips.push({
        type: 'requires',
        text: data.mode === 'any' ? `🔗 After ANY: ${shortNames}` : `🔗 After: ${shortNames}`,
        title: `${data.mode === 'any' ? 'Available after ANY of' : 'Available after ALL of'}: ${data.reqNames.join(separator)}`
      });
    } else {
      chips.push({ type: 'ready', text: '● With group', title: 'Available whenever this group is active' });
    }

    if (data.dependentNames.length) {
    ...
    const special = eventLabel(data.eventType);
    if (special) chips.push({ type: 'event', text: special, title: '' });

    if (data.unlocks.length) {
      chips.push({
        type: 'unlock',
        text: data.unlocks.length === 1 ? `🔓 ${data.unlocks[0]}` : `🔓 ${data.unlocks.length} groups`,
        title: `Unlocks: ${data.unlocks.join(', ')}`
      });
    }
  ...
  function clearLegacyPathClasses() {
    ...
    list.querySelectorAll('.forfeit-group.dep-path-active')
      .forEach(group => group.classList.remove('dep-path-active'));
  }

  function refresh() {
    scheduled = false;
    rebuilding = true;
    clearLegacyPathClasses();
    const byId = cardMap();
    const reverse = reverseMap(byId);
    byId.forEach(card => updateStatusRow(card, byId, reverse));
    rebuilding = false;
  }
  ...
  const observer = new MutationObserver(mutations => {
    ...
    const relevant = mutations.some(mutation => {
      if (mutation.type === 'attributes') return mutation.attributeName === 'data-id';
      if (mutation.type !== 'childList') return false;
      if (mutation.target.closest?.('.visual-status-row')) return false;
      return [...mutation.addedNodes, ...mutation.removedNodes].some(node => {
        if (node.nodeType !== 1) return false;
        const element = node;
        if (element.classList?.contains('visual-status-chip') || element.classList?.contains('tree-order-badge')) return false;
        return element.matches?.('.forfeit-editor-card,.forfeit-group') || element.querySelector?.('.forfeit-editor-card,.forfeit-group');
      });
    });
  ...
  document.addEventListener('input', event => {
    if (event.target.closest('.forfeit-editor-card')) schedule();
  });
