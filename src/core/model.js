(() => {
  ...
  const DEFAULT_CONFIG = {
    version: 1,
    settings: {
      title: 'Fortune Engine', minSpinSeconds: 6.5, maxSpinSeconds: 9.5, minTurns: 6,
      soundEnabled: true, showTextOnWheel: false, showProbabilities: true
    },
    levels: [
    ...
    forfeits: [
      { id:'starter-choice', name:'Choose a Challenge', icon:'🎯', color:'#4cc9f0', weight:4, levelId:'start', category:'Challenge', description:'Pick one quick challenge for the other player.', animation:'zoom', lifetime:{type:'forever',spins:3}, cooldown:1, eventType:'normal', timerSeconds:0, mystery:false, enabled:true, unlockLevels:[] },
      { id:'shoes-off', name:'Shoes Off', icon:'👟', color:'#8b5cf6', weight:3, levelId:'start', category:'Clothing', description:'Shoes come off. This also opens the clothing group.', animation:'pulse', lifetime:{type:'once',spins:3}, cooldown:0, eventType:'unlock', timerSeconds:0, mystery:false, enabled:true, unlockLevels:['clothing'] },
      { id:'mystery-one', name:'Mystery Challenge', icon:'🎁', color:'#ec4899', weight:2, levelId:'start', category:'Mystery', description:'A hidden challenge is revealed only after the wheel stops.', animation:'flash', lifetime:{type:'forever',spins:3}, cooldown:2, eventType:'normal', timerSeconds:0, mystery:true, enabled:true, unlockLevels:[] },
      { id:'unlock-tickling', name:'Unlock Tickling', icon:'🪶', color:'#f472b6', weight:1.5, levelId:'clothing', category:'Unlock', description:'The tickling group is now active.', animation:'confetti', lifetime:{type:'once',spins:3}, cooldown:0, eventType:'unlock', timerSeconds:0, mystery:false, enabled:true, unlockLevels:['tickling'] },
      { id:'tickle-short', name:'Quick Tickle', icon:'🪶', color:'#fb7185', weight:3, levelId:'tickling', category:'Tickling', description:'A short tickling challenge.', animation:'shake', lifetime:{type:'forever',spins:3}, cooldown:2, eventType:'normal', timerSeconds:0, mystery:false, enabled:true, unlockLevels:[] },
      { id:'chaos-key', name:'Chaos Key', icon:'🔑', color:'#f59e0b', weight:1, levelId:'tickling', category:'Unlock', description:'Unlocks the Chaos group.', animation:'confetti', lifetime:{type:'once',spins:3}, cooldown:0, eventType:'unlock', timerSeconds:0, mystery:true, enabled:true, unlockLevels:['chaos'] },
      { id:'spin-again', name:'Spin Again', icon:'🔁', color:'#22c55e', weight:1.5, levelId:'chaos', category:'Event', description:'No escape yet — spin one more time.', animation:'pulse', lifetime:{type:'forever',spins:3}, cooldown:1, eventType:'spinAgain', timerSeconds:0, mystery:false, enabled:true, unlockLevels:[] },
      { id:'randomize', name:'Chaos Shuffle', icon:'🎲', color:'#eab308', weight:1, levelId:'chaos', category:'Event', description:'Active wheel weights are randomly shifted for the rest of the session.', animation:'flash', lifetime:{type:'forever',spins:3}, cooldown:3, eventType:'randomize', timerSeconds:0, mystery:false, enabled:true, unlockLevels:[] }
    ],
    rules: [
      { id:'rule-demo', name:'Example combined unlock', mode:'all', conditionForfeitIds:['shoes-off','starter-choice'], unlockLevels:[], enabled:false }
    ]
  };

  const deepClone = value => JSON.parse(JSON.stringify(value));
  function makeId(prefix='item') {
    if (globalThis.crypto && typeof globalThis.crypto.randomUUID === 'function') return `${prefix}-${globalThis.crypto.randomUUID().slice(0,8)}`;
  ...
  function cleanColor(value,fallback='#64748b'){ const v=String(value||'').trim(); return /^#[0-9a-f]{6}$/i.test(v)?v:fallback; }
  ...
  function sanitizeConfig(input) {
    ...
    const levels = rawLevels.map((level,index)=>({
      ...
      name:String(level.name||`Group ${index+1}`).slice(0,40), icon:String(level.icon||'●').slice(0,10),
      color:cleanColor(level.color,'#64748b'), activeAtStart:Boolean(level.activeAtStart)
    }));
    if(!levels.length) levels.push({id:'start',name:'Start',icon:'✦',color:'#57d3ff',activeAtStart:true});
    ...
    const forfeits=rawForfeits.map((item,index)=>{
      ...
      return {
        ...
        name:String(item.name||`Forfeit ${index+1}`).slice(0,60), icon:String(item.icon||'🎯').slice(0,12),
        color:cleanColor(item.color,'#64748b'), weight:clampNumber(item.weight,.1,100,1),
        levelId:levelIds.has(item.levelId)?item.levelId:fallbackLevel,
        category:String(item.category||'Challenge').slice(0,30), description:String(item.description||'').slice(0,240),
        animation:['zoom','shake','pulse','flash','confetti'].includes(item.animation)?item.animation:'zoom',
        lifetime:{ type:['forever','once','spins'].includes(lifetime.type)?lifetime.type:'forever', spins:Math.round(clampNumber(lifetime.spins,1,999,3)) },
    ...
    const rules=rawRules.map((rule,index)=>({
      ...
      name:String(rule.name||`Rule ${index+1}`).slice(0,60), mode:rule.mode==='any'?'any':'all',
      conditionForfeitIds:Array.isArray(rule.conditionForfeitIds)?rule.conditionForfeitIds.filter(id=>forfeitIds.has(id)):[],
      unlockLevels:Array.isArray(rule.unlockLevels)?rule.unlockLevels.filter(id=>levelIds.has(id)):[], enabled:rule.enabled!==false
    }));
    return {
      version:1,
      settings:{
        fateDeck:window.FortuneFeatures.normalizeDeck(settings.fateDeck),
        reveals:window.FortuneFeatures.normalizeReveals(settings.reveals),
        cardImages:window.FortuneFeatures.normalizeCardImages(settings.cardImages),
        ...
        showTextOnWheel:Boolean(settings.showTextOnWheel), showProbabilities:settings.showProbabilities!==false
      }, levels, forfeits, rules
    };
  }

  function loadConfig(){
    try{ const raw=localStorage.getItem(CONFIG_KEY); if(raw) return sanitizeConfig(JSON.parse(raw)); }
    catch(error){ console.warn('Could not load saved configuration:',error); }
    return sanitizeConfig(deepClone(DEFAULT_CONFIG));
  }
  function saveConfig(config){ const clean=sanitizeConfig(config); localStorage.setItem(CONFIG_KEY,JSON.stringify(clean)); return clean; }

  function ensureCardState(session){
    if(!session.cardState||typeof session.cardState!=='object') session.cardState={};
    const s=session.cardState;
    s.respin=Math.max(0,Math.round(Number(s.respin)||0)); s.shorten=Math.max(0,Math.round(Number(s.shorten)||0));
    s.chaosNext=Boolean(s.chaosNext); s.doubleTimeNext=Boolean(s.doubleTimeNext);
  ...
  function createSession(config){
    const cfg=sanitizeConfig(config); const activeLevels={}; cfg.levels.forEach(level=>{activeLevels[level.id]=Boolean(level.activeAtStart);});
    const runtime={}; cfg.forfeits.forEach(item=>{runtime[item.id]={removed:false,cooldown:0,remainingSpins:item.lifetime.type==='spins'?item.lifetime.spins:null,weightMultiplier:1};});
    return ensureCardState({version:1,sessionId:makeId('session'),configSignature:signature(cfg),spinCount:0,wheelOrder:window.FortuneFeatures.shuffle(cfg.forfeits.map(item=>item.id)),activeLevels,runtime,history:[],undoStack:[],immunityTokens:0,doubleSpinTokens:0,cardState:{respin:0,shorten:0,chaosNext:false,doubleTimeNext:false,forcedForfeits:0,chaosBackup:{}}});
  }
  function loadSession(config){
    const cfg=sanitizeConfig(config); const sig=signature(cfg);
    try{ const raw=localStorage.getItem(SESSION_KEY); if(raw){ const parsed=ensureCardState(JSON.parse(raw)); if(parsed&&parsed.configSignature===sig) return parsed; } }
    catch(error){ console.warn('Could not load session:',error); }
    return createSession(cfg);
  }
  function saveSession(session){ localStorage.setItem(SESSION_KEY,JSON.stringify(ensureCardState(session))); return session; }
  function resetSession(config){ const session=createSession(config); saveSession(session); return session; }
  ...
  function configToXml(config){
    ...
    const reveal = window.FortuneFeatures.normalizeReveals(s.reveals);
    lines.push(`  <reveals envelopeCount="${reveal.envelopeCount}" envelopeHint="${reveal.envelopeHint}" autoplay="${reveal.autoplay}">`);
    reveal.envelopeGroups.forEach(id => lines.push(`    <group id="${xmlEscape(id)}" />`));
    lines.push('  </reveals>');
    lines.push('  <cardImages>'); Object.entries(s.cardImages).forEach(([id,data])=>lines.push(`    <image card="${id}">${data}</image>`)); lines.push('  </cardImages>');
  ...
  function xmlToConfig(xmlText){
    ...
    const revealNode=root.querySelector(':scope > reveals');
    const reveals=revealNode ? {envelopeCount:Number(attr(revealNode,'envelopeCount','3')),envelopeHint:attr(revealNode,'envelopeHint','none'),autoplay:boolAttr(revealNode,'autoplay',false),envelopeGroups:[...revealNode.querySelectorAll('group')].map(node=>attr(node,'id'))} : {};
  ...
  async function readXmlFile(file){ if(!file) throw new Error('No XML file selected.'); if(file.size>2_000_000) throw new Error('XML file is too large.'); return xmlToConfig(await file.text()); }

  window.FortuneModel={CONFIG_KEY,SESSION_KEY,DEFAULT_CONFIG:deepClone(DEFAULT_CONFIG),EVENT_TYPES:[...EVENT_TYPES],deepClone,makeId,sanitizeConfig,loadConfig,saveConfig,createSession,loadSession,saveSession,resetSession,signature,configToXml,xmlToConfig,downloadXml,readXmlFile,ensureCardState};
})();
